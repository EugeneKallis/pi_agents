...
jobsearch-json role:
    @cd agents/jobsearch && { [ -f ".env" ] && { set -a; . ".env"; set +a; } || true; } && \
    PI_CODING_AGENT_DIR=./.pi pi -p --provider opencode-go \
      "Search for {{role}} developer jobs. MANDATORY: call verify_job(url) on each promising listing and ONLY include jobs that return ACTIVE. Drop anything CLOSED or where the URL is a search/aggregate page (linkedin.com/jobs/search, indeed.com/jobs?, etc.) — find a real individual listing URL instead. SKIP ALL NEW JERSEY LOCATIONS (Jersey City, Newark, Hoboken, Weehawken, Clifton, any NJ city or county). Return ONLY a raw JSON object, no markdown code fences, no other text. The JSON must have exactly these keys: job_count (number), telegram (string with markdown), discord (object with content string and embeds array). discord.embeds items must have: title, description, salary, location_type (Remote or Hybrid or In-office or Unknown), url. Skip non-US jobs and ALL New Jersey jobs. Use search_jobs tool first, then web_search. Include exact URLs." \
      --no-session

# Aliases for common JSON searches
jobsearch-json-kdb: (jobsearch-json "KDB+")
jobsearch-json-go: (jobsearch-json "Golang")
jobsearch-json-python: (jobsearch-json "Python")
